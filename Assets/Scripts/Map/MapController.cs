...
public class MapController : MonoBehaviour
{
    ...
    void ChunkChecker()
    {
        ...
        {
            if (!Physics2D.OverlapCircle(currentChunk.transform.Find("Lift").position, checkerRadius, terrainMask))
            {
                noTerrainPosition = currentChunk.transform.Find("Lift").position;    //Left
                SpawnChunk();
            }
        }
        ...
        {
            if (!Physics2D.OverlapCircle(currentChunk.transform.Find("Lift Up").position, checkerRadius, terrainMask))
            {
                noTerrainPosition = currentChunk.transform.Find("Lift Up").position;  //Left up
                SpawnChunk();
            }
        }
        else if (pm.moveDir.x < 0 && pm.moveDir.y < 0)
        {
            if (!Physics2D.OverlapCircle(currentChunk.transform.Find("Lift Down").position, checkerRadius, terrainMask))
            {
                noTerrainPosition = currentChunk.transform.Find("Lift Down").position; //Left down
                SpawnChunk();
            }
        }
    }
